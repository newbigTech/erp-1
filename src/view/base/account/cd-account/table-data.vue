<template>
    <div class="c-table-data">
        <el-table
            ...
            <el-table-column label="CD状态" inline-template>
                <div :class="parseInt(row.enabled)===0?'loseStatus':'sucStatus'">{{row.enabled | filterEnabled}}</div>
            </el-table-column>
            ...
            <el-table-column label="操作" inline-template>
                <div>
                    ...
                    |
                    <span class="operate" @click="yz(row)" v-if="row.is_authorization===0">验证</span>
                    <span class="operate" v-else>验证成功</span>
                </div>
            </el-table-column>
        </el-table>
        <el-pagination
...
<script>
    import {api_system_states,api_cd_account_check} from '@/api/cd-account';
    export default {
        data() {
            return {
            }
        ...
        methods:{
            ...
            edit_compile(row){
                this.$emit('edit-compile',row);
            },
            edit_authorization(row){
                this.$emit('edit-authorization',row);
            },
            yz(row){
                this.$http(api_cd_account_check,{id:row.id}).then(res=>{
                    this.$message({type:'success',message:res.message||res,duration:1500})
                    this.$emit('search')
                }).catch(code=>{
