<template>
    <page-dialog :title="title" v-model="show" size="small"
        ...
        <el-form :model="smtData"
                 label-width="190px"
                 ref="smtData">
            <el-form-item  label="系统状态：">
                <el-col :span="14">
                    <el-switch v-model="smtData.is_invalid"
                               :on-text="status[0]"
                               on-color="#20a0ff"
                               :off-text="status[1]"
                               off-color="#C0CCDA"
            ...
            <el-form-item label="抓取速卖通Listing功能：">
                <el-col :span="14">
                    <el-select v-model="smtData.download_listing">
            ...
            <el-form-item label="抓取速卖通订单功能：">
                <el-col :span="14">
                    <el-select v-model="smtData.download_order">
            ...
            <el-form-item label="抓取速卖通站内信功能：">
                <el-col :span="14">
                    <el-select v-model="smtData.download_message">
                        <el-option
                                :key="item.value"
                                v-for="item in crawl"
                                :label="item.label"
            ...
            <el-form-item label="同步发货状态到速卖通功能：">
                <el-col :span="14">
                    <el-select v-model="smtData.sync_delivery">
                        <el-option v-for="item in crawl"
                                   :key="item.value"
                                   :label="item.label"
                                   :value="item.value">
            ...
            <el-form-item label="同步健康数据：">
                <el-col :span="14">
                    <el-select v-model="smtData.download_health">
                        <el-option v-for="item in dowloadLists"
                                   :key="item.value"
                                   :label="item.label"
                                   :value="item.value"></el-option>
...
<script>
    ...
    export default{
        ...
        methods:{
            open(){
                this.smtData.is_invalid = this.smtData.is_invalid === 1;
            },
            cloce(){

            },
            keep(){
                this.$emit('set-clich',this.smtData);
            },
            changStatus(){//-----------------------修改启用或禁用事件
                let statusTitle = this.smtData.is_invalid ? '停用' : '启用';
                this.$confirm('您将'+statusTitle+'，确认此操作吗？', '提示', {
                    ...
                    this.smtData.is_invalid = !this.smtData.is_invalid;
                });
            },
        },
        watch:{
