    ...
    <page class="p-dialog">
        <page-dialog :title="title" v-model="showDialog" size="small" :close-on-click-modal="false" @close="close">
            ...
            <div slot="footer" class="dialog-footer">
                <el-button size="mini" type="primary" @click="get_shift">确定</el-button>
...
<script>
    ...
    import  {api_aliexpress_account_health_set_more} from '@/api/monitor-manage'
    export default {
        data() {
            return {
                title:'批量修改监控值',
        ...
        methods: {
            get_shift(){//确定
                ...
                account_ids = this.SelectionData.map(row=>row.account_id);
                monitoringData['account_ids'] =  account_ids.join(',');
                this.$http(api_aliexpress_account_health_set_more,monitoringData).then(res=>{
                    this.SelectionData.forEach(row=>{
                        delete monitoringData.account_ids;
                        row.goal = clone(monitoringData);
                    });
            ...
            close(){
                this.tableData_.to_punish.forEach(row=>row.name = '');
                this.tableData_.to_serve_score.forEach(row=>row.name = '');
            },
        },
        watch: {
            value(val){
