<template>
    <ui-table class="order-rule-data-table add-data-tables"
              v-model="checkAll"
              :heads="[
        ...
        <tbody v-if="tables.lists.length>0">
            <tr class="tbody"
                ...
                <td class="btns">
                    <permission tag="span" class="mini btn" @click="lookup(item)"  :route="apis.url_update">查看编辑</permission>
                    <permission tag="span" class="mini btn" @click="copy(item)"  :route="apis.url_copy">复制</permission>
                    <permission tag="span" class="mini btn" @click="del(item)" :route="apis.url_del">删除</permission>
                    <permission tag="span" class="mini btn" @click="log(item)" :route="apis.url_del">日志</permission>
                </td>
            </tr>
        </tbody>
        <tbody v-else>
...
<script>
    ...
    import {api_del,api_copy,api_change_status,api_get_log,
        url_update,url_del,url_change_status,url_copy,url_get_log
...
//                        this.showDelete = false;
            ...
            log(item){
                this.$http(api_get_log, item.id).then(res=>{
        ...
        computed:{
            checkAll:{
                get(){
                    if(!this.tables.lists||this.tables.lists.length<=0)return false;
                    return !this.tables.lists.find(row=>!row.isCheck)
                },
                set(val){
                    this.tables.lists = this.tables.lists.map(row=>{
