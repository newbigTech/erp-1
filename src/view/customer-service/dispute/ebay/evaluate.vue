        ...
        <table class="template" v-for="(item, index) in formData">
            <tr>
                <td style="width:300px;text-align: right">
                    <span v-if="!!item.channel_item_id">{{index + 1}}: </span>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            toRefund(){
                this.$confirm(`您将重新退款?`,'提示',{
                    ...
                    this.$http(api_orders_order_renew_refund,{order_id:this.cancelId}).then(res=>{
            ...
            markRefund(){
                this.$confirm(`您将标记为已退款?`,'提示',{
                    ...
                    this.$http(api_mark_refund_failed,{order_id:this.cancelId}).then(res=>{
                        this.$emit('refund-status-change',res.refund_status);
                        this.$message({type:'success',message:res.message||res});
                    }).catch((err)=>{
                        this.$message({type:'error',message:err.message})
        ...
        components: {
            contactBuyer: require('../../../order/local/contact-buyer.vue').default,
            message: require('../../../order/local/message.vue').default,
