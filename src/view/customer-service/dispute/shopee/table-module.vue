<template>
    <div class="p-shopee-table-module">
        <el-table
            ...
            <el-table-column label="订单号" inline-template>
                <div>
                    <permission tag="span" type="text"
                                ...
                                :route="searchData.type === 1?apis.url_shopee_look_cancel:apis.url_shopee_look_return"
                                :title="row.ordersn === '' ? '没有订单号，无法查看' : row.ordersn"
                                :disabled="row.ordersn === ''">
                        {{row.ordersn ? row.ordersn : '--'}}
                    </permission>
                </div>
            </el-table-column>

            <el-table-column label="平台退货单号" inline-template>
                <div>
                    <ui-tip :content="row.returnsn?row.returnsn:'--'" :width="98"></ui-tip>
                </div>
            </el-table-column>
            ...
            <el-table-column label="金额总计" inline-template>
                <div>
                    <ui-tip :content="`${row.amount}${row.currency}`" :width="98"></ui-tip>
                </div>
            </el-table-column>

            <el-table-column label="原因" inline-template>
                <div>
                    <ui-tip :content="row.reason_text" :width="98"></ui-tip>
                </div>
            </el-table-column>
            ...
            <el-table-column label="纠纷状态" inline-template>
                <div>
                    <ui-tip :content="row.status_code_text" :width="98"></ui-tip>
                </div>
            </el-table-column>

            <el-table-column label="平台状态" inline-template>
                <div>
                    <ui-tip :content="row.status" :width="98"></ui-tip>
                </div>
            </el-table-column>

            <el-table-column inline-template label="最迟回应时间" width="150" sortable="custom" prop="due_date">
                <div>
                    <span v-if="row.status_code !== 1">已响应</span>
                    <span v-if="row.status_code === 1&&(row.due_date*1000<new Date().getTime())" style="color:red">已超时</span>
                    <times :time="row.due_date" v-if="row.status_code === 1&&(row.due_date*1000>new Date().getTime())"></times>
                </div>
            </el-table-column>
            ...
            <el-table-column label="操作" inline-template>
                <div>
                    ...
                    <template v-if="row.status_code === 1">
                        <permission tag="span" :route="searchData.type === 1?apis.url_shopee_look_cancel:apis.url_shopee_look_return">|</permission>
                        <permission tag="ElButton"
                                    type="text"
                                    :route="searchData.type === 1?apis.url_shopee_look_cancel:apis.url_shopee_look_return"
                                    class="operate"
...
<script>
    import {
        url_shopee_look_return,
        url_shopee_look_cancel
    } from "../../../../api/dispute"
    import {
        url_look_over
    } from '@/api/after-sale';
