<template>
    <span :class="!!$slots.default?'':'rule1'">
        <div :class="$slots.default?'rule1-parent' : ''" :title="form.title" v-if="polyType(form.child)&&!form.child.url">
        ...
        <el-select class="inline condition" v-if="condition.length > 1&&polyType(form.child)&&!form.child.url" v-model="newSelect" placeholder="全部">
        ...
        <div v-if="!polyType(form.child)&&form.child.url" style="margin-top:5px">
            ...
            <el-card class="rule1-select-tag">
                <el-tag v-for="tag in selected_id" :key="tag">
                    {{tag.shortname}}
        ...
        <div class="rule1-child" v-if="$slots.default&&polyType(form.child)&&!form.child.url&&showChild">
            <slot></slot>
        </div>
