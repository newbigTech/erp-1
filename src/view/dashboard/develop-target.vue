<template>
    <ui-card height="330px">
        ...
        <el-table
            ...
            <el-table-column label='目标（个）' prop="target_amount"></el-table-column>
            <el-table-column label='已完成(个)' prop="actual_amount"></el-table-column>
            <el-table-column label='进度' inline-template>
                <div>{{row.rate}}%</div>
            </el-table-column>
        </el-table>
        <develop-target-detail v-model="dialog"
                               :cur-date="curDate"></develop-target-detail>
    </ui-card>
</template>
<style lang="stylus">
...
<script>
    import {api_develop_monthly_target} from '@/api/monthly-target';
    export default {
        data() {
            return {
                tableData:[],
        ...
        components: {
            ...
            developTargetDetail:require('./develop-target-detail.vue').default,
        }
    }
</script>
