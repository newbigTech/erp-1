...
export const print = function (bool, flag) {
    ...
    this.print_url = this.batchData.callback ? config.apiHost + this.batchData.callback : '';
    this.print_html = '';
    let back = '<div style="overflow:hidden;position: relative;margin:0 auto;page-break-before:always;' + `height:${(parseInt(this.templateData.size.height) * (72 / 25.4)).toFixed(4)}pt;` + `width:${(parseInt(this.templateData.size.width) * (72 / 25.4)).toFixed(4)}pt;` + '">';
    let dashed = '<div style="overflow:hidden;position: absolute;margin:auto;page-break-before:always;border:1px dashed red;' + `height:${(parseInt(this.templateData.size.height) * (72 / 25.4)).toFixed(4)}pt;` + `width:${(parseInt(this.templateData.size.width) * (72 / 25.4)).toFixed(4)}pt;left:0;right:0;bottom:0;top:0;` + '">';
    let num = 1;
    window.clone(this.templateData.temp_data).forEach(row => {
        let tag;
        let single = `height:${px2pt(row.style.height)}pt;width:${px2pt(row.style.width)}pt;font-family:${row.style.fontFamily};font-weight:${row.style.fontWeight};font-size:${px2pt(row.style.fontSize)}pt;text-align:${row.style.textAlign};left:${px2pt(row.style.left)}pt;top:${px2pt(row.style.top)}pt;word-break:break-all;`;
        if (row.type === 1) {
    ...
    if (bool) {
        ...
        if (flag) {
            this.numberList = this.batchData.identification.join();
            this.numberList = this.numberList.replace(/\,/g, '_a_b_c_d_');
