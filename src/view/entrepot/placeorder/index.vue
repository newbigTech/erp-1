<template>
    <page class="p-index">
        ...
        <label-item label="请选择本地仓库：" class="mb-xs mt-sm">
...
<script>
    ...
    import {
        ...
        placeorder, batchUpload, api_warehouse_channel, api_placeorder_package_statuses,
    ...
    export default {
        ...
        methods: {
            ...
            init_warehouse() {
                this.$http(api_warehous_local, {need_transit: 0}).then(res => {
            ...
            init() {//获取数据
                if (!!this.warehouse_id && this.buttonsChannel.length > 0 && this.buttonsPackages.length > 0 && this.buttonsUpload.length > 0) {
                    ...
                    this.$http(placeorder, this.parmas).then(res => {
        ...
        computed: {
            ...
            canCheckAll() {
                if(this.buttonsPackages.length !== 0) {
                    return ![16,17].includes(this.buttonsPackages[this.packageStatus].value);
                } else {
                    return true;
                }
            }
