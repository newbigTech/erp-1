    ...
    <div class="p-return-shelves">
        <ui-card>
            <div slot="header" class="ui-card-header">
                ...
                <request-button :disabled="canCreate" req-key="create_order" :request="create_order">生成上架单</request-button>
            </div>
            <el-table
                    :data="tableData"
                    v-resize="{height:45}"
                ...
                <el-table-column
                        ...
                        label="周转篮号">
                    <div>
                        <ui-tip :content="row.number" :hiLite="row.quantity_can<Number(row.quantity)" :width="98"></ui-tip>
                    </div>
                </el-table-column>
                <el-table-column
                        inline-template
                ...
                <el-table-column
                    ...
                    <template>
                        <div @dblclick="db_quantity(row)" class="quantity-hover" title="双击可手动更改数量">
                            <span v-if="row.isShow" :class="{lineColor:row.quantity_can<Number(row.quantity)?true:false}">{{row.quantity}}</span>
                            <integer-input v-if="!row.isShow" v-model="row.quantity" :min="1" class="inline width-mini" @blur="quantityBlur(row)"></integer-input>
                ...
                <el-table-column
                        ...
                        label="可上架数">
                    <div>
                        <ui-tip :content="row.quantity_can" :hiLite="row.quantity_can<Number(row.quantity)" :width="98"></ui-tip>
                    </div>
                </el-table-column>
                <el-table-column
                        width="270"
                        ...
                        label="名称（规格）">
                    <div>
                        <ui-tip :content="row.spu_name" :hiLite="row.quantity_can<Number(row.quantity)" :width="98"></ui-tip>
                    </div>
                </el-table-column>
                <el-table-column
                        inline-template
                        label="货位号">
                    <div>
                        <ui-tip :content="row.warehouse_cargo_code" :hiLite="row.quantity_can<Number(row.quantity)" :width="98"></ui-tip>
                    </div>
                </el-table-column>
                <el-table-column
                        inline-template
                        label="分区">
                    <div>
                        <ui-tip :content="row.warehouse_area_type" :hiLite="row.quantity_can<Number(row.quantity)" :width="98"></ui-tip>
                    </div>
                </el-table-column>
                <el-table-column
                        inline-template
...
<script>
    import {api_get_shelves, api_post_shelves,api_reback_shelves_order_quantity,api_reback_shelves_order} from '../../../api/return-shelves'
    ...
    export default {
        ...
        methods: {
            ...
            create_order(){
                ...
                return this.$http(api_reback_shelves_order, {data:data},{timeout:INFINITE}).then(res=>{
