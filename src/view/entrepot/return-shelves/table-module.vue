    ...
    <div>
        <el-table
            ...
            <el-table-column
                ...
                label="数量（上架数/总数）">
                <div>
                    {{row.quantity_left===0?'--':row.quantity_left}}/{{row.quantity_real===0?'--':row.quantity_real}}
                </div>
            </el-table-column>
            <el-table-column
                    inline-template
...
<script>
    import {api_get_return, api_look_shelves} from '../../../api/return-shelves'
    export default {
        data(){
            return {
                loading:false,
        ...
        methods: {
            ...
            look(row){
                ...
                this.$http(api_look_shelves, row.id).then(res=>{
