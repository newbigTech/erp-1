<template>
    <page-dialog title="添加入库记录" v-model="show" size="large"
        ...
        <div>
            ...
            <label-item label="来源单号：" class="ml-sm mr-sm">
                <el-input :placeholder="`请输入来源单号...`" v-model="addData.purchase_order_id" class="inline"></el-input>
            </label-item>
            <label-item label="入库类型：" class="ml-sm mr-sm">
                <el-select class="inline width-sm" v-model="addData.type">
                    <el-option v-for="item in inputList"
                               :key="item.value"
                               :value="item.value"
                               :label="item.label"></el-option>
                </el-select>
            </label-item>
            <div class="inline ml-sm">备注：</div>
            <el-input
                    class="inline"
                    style="width: 200px"
                    :rows="1"
                ...
                <div class="mb-sm mt-sm">
                    <div class="fl">
                        ...
                        <permission tag="ElButton" type="primary"
                                    :route="apis.url_stock_in_import"
            ...
            <el-table
                ...
                <el-table-column
                    ...
                    <el-input
                            type="number"
                            :min="0"
                            @blur="blur_price(row.in_cost_price)"
                            v-model="row.in_cost_price">
                    </el-input>
                </el-table-column>
                <el-table-column
                        inline-template
...
<script>
    import {api_post_stock, stock_types, api_get_goods, url_stock_in_import} from '@/api/stock-in';
    ...
    export default {
        ...
        data() {
            return {
                ...
                inputList: [{label: '其他入库', value: 18}, {label: '其他系统退货入库', value: 19}],
                warehouses: [],
                show: this.value,
                textarea: "",
                goodsDialog: false,
            ...
            id_enter() {// ----------------快速添加
                ...
                this.$http(api_get_goods, {sku: sku, warehouse_id: warehouse_id}).then(res => {
                    ...
                    if (!!list) {
                        ...
                        let cenData = {
                            sku: res.sku,
                            in_spu_name: res.spu_name,
                            quantity: 1,
                            cargo_code: res.cargo_code,
                            in_cost_price: res.in_cost_price,
                            sku_id: res.sku_id,
                        };
                        this.editData.push(cenData);
                        this.enterVal = '';
                    }
...
//                this.purchase_prices[row.sku_id] = Number(this.purchase_prices[row.sku_id]).toFixed(2);
            ...
            blur_price(val) {
                if (val === '') return this.$message({type: "warning", message: '单价不能为空'});
            },
