...
export const print = function (bool, flag) {
    ...
    let diff = 0;
    if(this.templateData.id === 32) {
        diff = 16;
    }
    let back = '<div style="position: relative;margin:0 auto;page-break-before:always;' + `height:${(parseInt(this.templateData.size.height) * (72 / 25.4)).toFixed(4) - diff}pt;` + `width:${(parseInt(this.templateData.size.width) * (72 / 25.4) - 8).toFixed(4)}pt;` + 'overflow:hidden;' + '">';
    let dashed = '<div style="position: relative;margin:0 auto;page-break-before:always;border:1px dashed red;' + `height:${(parseInt(this.templateData.size.height) * (72 / 25.4) - diff).toFixed(4)}pt;` + `width:${(parseInt(this.templateData.size.width) * (72 / 25.4) - 8).toFixed(4)}pt;` + '">';
    window.clone(this.templateData.temp_data).forEach(row => {
        let tag;
        let single = `height:${px2pt(row.style.height)}pt;width:${px2pt(row.style.width)}pt;font-family:${row.style.fontFamily};font-weight:${row.style.fontWeight};font-size:${px2pt(row.style.fontSize)}pt;text-align:${row.style.textAlign};left:${px2pt(row.style.left)}pt;top:${px2pt(row.style.top)}pt;white-space: nowrap;`;
        if (row.type === 1) {
    ...
    for (let i = 0; i < this.printData.print_num; i++) {
        this.numberList.push(this.printData.code + '-' + i);
    }
    if (bool) {
        //将要打印的内容拼接起来
        this.print_type.push('html');
        if(this.templateData.id === 32) {
        // if (this.templateData.id === 78) {
            this.print_direction.push('transverse');
        } else {
            this.print_direction.push('vertical');
        }
        this.print_html = head + prints + bottom;
        this.print_content.push(this.print_html);
//判断传入的参数flag是否为true如果为true就打印，当循环完成时flag才为true
        //因为for循环提交表单的时候只会提交最后一次所以这里需要一个参数flag当循环完成时提交一次表单
        if (flag) {
            this.numberList = this.numberList.join('_a_b_c_d_');
            this.print_content = this.print_content.join('_a_b_c_d_');
            this.print_type = this.print_type.join('_a_b_c_d_');
            this.print_direction = this.print_direction.join('_a_b_c_d_');
            if (this.templateData.id === 32) {
            // if (this.templateData.id === 78) {
                let height = this.print_width;
                this.print_width = this.print_height;
            ...
            let data = {
                ...
                Direction: this.print_direction,
            };
