        ...
        <page-dialog :title="title" v-model="dialog" size="large" :close-on-click-modal="false" @change="change_dialog">
            ...
            <div slot="footer" class="dialog-footer">
                <permission tag="ElButton"
                            :route="apis.url_save_allot"
                            size="mini" type="primary" @click.native="submit">确认分配
                </permission>
                <el-button size="mini" @click.native="dialog=false">取消</el-button>
            </div>
            <!--<select-plan v-model="selectShow" :selset-id="selectId" @update-id="update_id"></select-plan>-->
        </page-dialog>
    </page>
</template>
<style lang="stylus">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            update_id(id) {
                id.forEach(res => {
                    this.$emit('refresh-data', this.allotData.id, res.id);
                });
            },
            submit() {
                if (!this.validate()) {
