        ...
        <page-dialog :title="title" v-model="dialog" :size="size" :close-on-click-modal="false" @change="change_dialog">
            <div style="width: 100%;text-align: center;">
                <label-item label="预警数量：" class="mb-xs label_one" v-if="valueShow===1">
                    <integer-input :min="0" v-model="number"></integer-input>
                </label-item>
                ...
                <label-item label="借调数量：" class="mb-xs dis_block" v-if="valueShow===3">
                    <integer-input :min="1" v-model="quantity"></integer-input>
                </label-item>
                <div v-if="valueShow===2" style="text-align: left;">
                    <el-row>
                        <label-item label="备货平台：" class="inline">
                            <el-select class="inline width-sm" v-model="channel_id" placeholder="请选择" :disabled="isFba">
                                <el-option
                                        v-for="item in channelList"
                                        :key="item.value"
                                        :label="item.label"
                    ...
                    <el-button size="mini" type="primary" class="mb-mini mt-mini" @click="delete_data">删除<span
                            v-if="this.selectIds.length">({{deleteNumber}})</span></el-button>
            ...
            <div slot="footer" class="dialog-footer">
                <request-button req-key="alertAdd" @click="save" v-if="valueShow===1">确定</request-button>
                <request-button req-key="applyAdd" @click="submit" v-if="valueShow===2">提交</request-button>
                <request-button req-key="lendAdd" @click="get_lend" v-if="valueShow===3">确定</request-button>
        ...
        methods: {
            ...
            get_channel() {
                this.$http(api_get_channel, {}).then(res => {
                    ...
                    if (this.valueShow === 2 && this.isFba) {
                        this.channel_id = 2;
                    }
                    console.log('状态：', this.valueShow === 2, this.isFba);
                }).catch(code => {
