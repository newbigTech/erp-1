<template>
    <el-row class="aliexpress-has-published">
        ...
        <el-row class="mb-sm ml-sm mt-sm">
            <permission tag="request-button" :mintime="200" req-key="url_aliexpress_batch_del" @click="batchdel" :route="apis.url_aliexpress_batch_del">批量删除</permission>
        </el-row>
        <!--table数据-->
        <ui-table
            ref="table"
            ...
            <template v-for="(data, index) in tableData">
                <tr :class="[data.show ? 'active' : '',data.heighLight?'active-color':'',]" @click="addClass(index)" :key="data.id">
                    ...
                    <td><times :time="data.plan_publish_time"></times></td>
...
<script>
    import {api_aliexpress_list_wait,api_aliexpress_account,api_aliexpress_product_status,api_aliexpress_expire_search,api_aliexpress_batch_del,api_aliexpress_sale_list,url_aliexpress_batch_del} from "../../../../api/publish-smt"
    ...
    export default{
        ...
        methods:{
            ...
            init(){
                ...
                this.$http(api_aliexpress_list_wait,data).then(res=>{
            ...
            batchdel(){
                ...
                if(data.length===0){
                    this.$reqKey('url_aliexpress_batch_del',false);
