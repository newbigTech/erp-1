    ...
    <div style="margin: 0 10px">
        <el-tabs v-model="tabValue" type="border-card" closable @tab-remove="removeTab">
            <el-tab-pane v-for="(item, index) in tabData"
                         ...
                         :name="item.tab_label">
                <!-- 基本信息 -->
                <account-tab-base :account-info="item" @initial-uppercase="initial_uppercase(item)" :kinds="kinds"></account-tab-base>
                <!-- sku设置 -->
...
<script>
    import {api_joom_dangerous_kind} from "@/api/joom-set-classify"
    export default{
        data(){
            return {
                tabData: [],
        ...
        methods: {
            get_kinds(){
                this.$http(api_joom_dangerous_kind).then(res=>{
                    this.kinds=res;
                }).catch(code => {
