<template>
    <div class="p-sales-tabel">
                ...
                <tr :class="[data.show ? 'active' : '',data.heighLight?'active-color':'',data.listing_status===5?'redborder':'']" @click="addClass(index)"
                    ...
                    <td>
                        <div style="position: relative">
                            <span v-if="!data.goods_id" @click="bind_sku(data, index)" class="operate">
                            ...
                            <ui-tip v-else :content="data.spu"></ui-tip>
        ...
        <bind-spu v-model="bindVisible" :channel-sku="channelSku" :local-sku="localSku" type="pandao" @refresh="valids_joom"></bind-spu>
    </div>
</template>
<style lang="stylus">
...
<script>
    import {api_record_logs,url_record_logs,url_record_Synchronize,api_record_Synchronize,url_record_drop,api_record_drop,} from '@/api/mymall'
        ...
        data(){
            return{
                ...
                btnList:[//----------------------------------------------操作按钮
                    ...
                    {name:'批量下架',clickName:'',status:2,api:url_record_drop},
                ],
        ...
        methods:{
            ...
            async bind_sku(data, index){
               ...
               // let res = await this.$http(api_joom_list_variant,{joom_product_id: data.id,});
                let  res=window.clone(data.variants);
            ...
            // 关联sku后列表的变化
            valids_joom(val){
                this.tableData[this.index].spu = this.spu;
            },
            cur_click(row){ //连接跳转
            ...
            operate_offline_data(datas,obj){
                this.$http(api_record_drop,obj).then(res=>{
                    this.$message({
                        type:"success",
                        message:res.message||res
                    });
