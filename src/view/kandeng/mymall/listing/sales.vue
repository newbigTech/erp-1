...
<script>
  import  {api_get_sellers,api_publish_on_sale} from "@/api/mymall"
    export default {
        data(){
            return{
                paramList:{
        ...
        methods:{
        	init(){
        		...
        		Object.assign(data,{enabled:1,publish_status:1,nTime:'date_uploaded'});
                this.loading = true;
        		this.$http(api_publish_on_sale,data).then(res=>{
            ...
            // 排序
            sort_click(val){
                this.paramList.field = val.order_type;
                this.paramList.order = val.order;
        ...
        components:{
            search: require('./sales-search.vue').default,
            tabelModule: require('./sales-tabel-module.vue').default,
        }
    }
</script>
