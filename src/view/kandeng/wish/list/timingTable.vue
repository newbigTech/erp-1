<template>
    <el-row class="wish-time-published">
        <search-card @search="search" @enter="search" :params="searchData" :clears="clears">
            <el-select class="inline s-width-default" v-sf.nType v-model="searchData.nType" >
            ...
            <label class="inline">是否定时刊登：</label>
            ...
            <label class="inline">更新时间：</label>
            <el-date-picker class="inline  date"  v-sf.start_time v-model="searchData.start_time"  :picker-options="pickerstart" type="date" placeholder="开始时间">
        ...
        <div class="mt-sm ml-sm mb-sm">
            <permission tag="request-button" :mintime="200" req-key="url_wish_del1" @click.native='clicked' :route="apis.url_wish_del">批量删除</permission>
        </div>
        <!--table数据-->
        <ui-table :has-data="publishList.length>0" v-model="checkAll" :body-height="38" @check="check_all" v-loading="loading" element-loading-text="玩命加载中..." ref="table"
                      :heads="[
                        {isCheck:true,width:2,},
                        {label:'产品图片',size:75},
                        {label:'平台SKU/本地SPU',width:14},
                        {label:'刊登标题',width:11},
                        {label:'产品标签',width:13},
                        {label:'账号简码',width:10},
                        {label:'定时刊登时间',width:10},
                        {label:'销售员',width:6},
                        {label:'销售价',width:7},
                        {label:'运费',width:7},
                        {label:'已售量',width:6},
            ...
            <template v-for="(data, index) in publishList">
                <tr :class="[data.show ? 'active' : '',data.heighLight?'active-color':'']" @click="addClass(index)" :key="data.id">
                    ...
                    <td>
                        ...
                        <div class="inline">
                            <div class="wish-time-td-text" :title="data.parent_sku">{{data.parent_sku}}</div>
                            <div class="wish-time-td-text" :title="data.local_spu">{{data.local_spu}}</div>
                        </div>
                    </td>
                    <td><ui-tips :content="data.name"></ui-tips></td>
                    <td><ui-tips :content="data.tags"></ui-tips></td>
                    <td><ui-tips :content="get_account_code(data)"></ui-tips></td>
                ...
                <template v-if="data.show">
                    ...
                    <tr v-for="item in data.skus" :key="item.sku" class="variant">
                        ...
                        <td>
                            <div v-if="item.skumap">{{item.skumap.sku.status | localStatus}}</div>
                        </td>
                        <td><ui-tips :content="item.message"></ui-tips></td>
                    </tr>
                </template>
            </template>
            </tbody>
...
//                   {name:'批量编辑',clickName:'',status:1},
        ...
        computed:{
            optionList(){
                return  operateList.filter(row=>{
                    return this.$hasPermission(row.api)&&row.stauts.includes(4)
                })
            }
        },
        created(){
        ...
        methods:{
            get_path,
            get_account_code(data){
                return data.account?data.account.code:'';
            },
            main_img(image,size){
            ...
            clicked(){//------------------------------------批量删除
                ...
                if(arr.length===0){
                    this.$reqKey('url_wish_del1',false);
                    this.$message({
                        message: '请选择删除的商品',
                        type: 'warning'
                    });
            ...
            operate_del(datas){
                if(datas instanceof Array){
                    ...
                    let i = this.publishList.indexOfFun(datas,function (old,datas) {
