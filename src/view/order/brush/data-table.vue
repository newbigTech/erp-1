<template>
    <div class="relative">
        <color-tip></color-tip>
        <el-row>
            <ui-table v-loading="isLoad" :first-loading="firstLoading" element-loading-text="玩命加载中..."  :has-data="tables.lists.length>0" class="local" v-model="checkAll" @check="change_all" @sort-click="sort_click" :body-height="38"
                :heads="[
                    ...
                    {label:'支付总额',width:5,isSort:true,type:'desc'},
                    {label:'运输方式',width:7},
                    {label:'发货仓库',width:6},
                    {label:'发货状态',width:5},
                    {label:'目的地',width:6,isSort:true,type:'desc'},
                    {label:'下单时间',width:6,isSort:true,type:'desc'},
                    {label:'最迟发货时间',width:8,isSort:true,type:'desc'},
                    {label:'同步状态',width:5},
                    {label:'操作',width:8}
                ]"
            >
                <tbody>
                    <template v-for="(order,index) in tables.lists">
                        <tr  :class="[order.show ? 'active' : '',clickData[index]?'active-color':'',order.abnormal?'problem-color':'',order.status===4294967295?'cancel-color':'']" @click="addClass(index)">
                            <td>
                            ...
                            <!--下单时间-->
                            <td>
                                <times v-if="order.order_time" :time="order.order_time"></times>
                                <label v-else>未下单</label>
                            </td>
                            <!--最迟发货时间-->
                            <td>
                                <time-out :time="order.uploaded_deadline"
                                          :cur-status="order.shipping_status"></time-out>
                            </td>
                            <!--同步状态-->
                            <td>
                                <ui-tip :content="order.synchronize_latest" :width="98"></ui-tip>
                            </td>
                            <td>
...
<script>
    import formMdf from '../local/form-mdf.vue';
    ...
    export default{
        ...
        methods:{
            ...
            action(data,options){
                ...
                this.brushForm.synchronize_shipping_id = data.synchronize_shipping_id===0?"":data.synchronize_shipping_id;
                this.$set(this.brushForm,'synchronize_shipping_time',data.synchronize_shipping_time?new Date(data.synchronize_shipping_time*1000):"");
                this.$set(this.brushForm,'synchronize_tracking_number',data.synchronize_tracking_number?data.synchronize_tracking_number:"");
...
//            升序
            ...
            change_all(val){
                ...
                this.$emit("change-all",this.activeData);
            },
//            表格升降序
            sort_click(val){
        ...
        components:{
            ...
            brushShipments:require('../brush/brush-shipments.vue').default,
