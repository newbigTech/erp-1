        ...
        <ui-table v-loading="loading"
            ...
            <template v-for="(data, index) in dataTable">
                <tr class="line" :class="[data.show ? 'active' : '',data.clickData?'active-color':'']"
                    ...
                    <!--买家姓名-->
                    <td>
                        <ui-tip :content="data.customer_first_name" :width="72"></ui-tip>
                    </td>
                    <!--目的地-->
                    <td>
                    ...
                    <!--出售价-->
                    <td>
                        <ui-tip :content="data.amount" :width="88"></ui-tip>
                    </td>
                    <!--支付总额-->
                    <td>
                        <ui-tip :content="data.validated_total_amount" :width="88"></ui-tip>
                    </td>
                    <!--付款时间-->
                    <td>
                        <ui-tip :content="data.order_created|dateFilter" :width="78"></ui-tip>
                    </td>
                    <!--平台发货状态-->
                    <td>
                        <ui-tip v-if="data.push_status>0" :content="data.push_status|dateFilter"
                                :width="88"></ui-tip>
                        <div v-else-if="data.push_status===0"><span>未发货</span></div>
                    </td>
                    <!--订单状态-->
                    <td>
                    ...
                    <td>
                        <permission
                            tag="span"
                            :route="apis.url_api_order_cd_check"
                            class="operate"
...
<script>
    ...
    import {api_order_cd_check, api_cd_orders_export, url_cd_orders_export} from '../../../../api/order_cd';
    import {url_api_order_cd_check} from '../../../../api/order_cd';
    import uiTable from "../../../../components/ui-table.vue"

    export default {
        permission: {
            url_api_order_cd_check,
            url_cd_orders_export
        },
        data() {
            return {
                orders: false,
        ...
        methods: {
            ...
            lookOver(row) {//----------------------查看
                this.orders = true;
                this.checkList = {};
                this.$http(api_order_cd_check, row.id).then(res => {
                    this.checkList = res;
