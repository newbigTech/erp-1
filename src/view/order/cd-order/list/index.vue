...
<script>
    import {api_order_cd_list,api_order_cd_status} from'../../.././../api/order_cd';
    export default{
        page:{
            devinfo:{
                frontEnd:'覃宏峰',
        ...
        methods:{
            get_status(){
                ...
                data.status = -1;
                this.$http(api_order_cd_status,data).then(res=>{
                    res.forEach(btn=>{
            ...
            init(){
                this.loading=true;
                this.dataTable=[];
                this.$http(api_order_cd_list,this.get_params()).then(res => {
            ...
            sort_click(val){
                switch (val.label){
                    case "支付总额":
                        this.searchData.sort_type="validated_total_amount";
                        break;
                    case "付款时间":
        ...
        components:{
            searchList:require('./searchList').default,
