        ...
        <table class="form-item-product">
            <template v-for="(item,index) in form">
                <tr :class="[item.detail_goods.length>0?'success-color-title':'error-color-title']">
                    ...
                    <td>物流属性</td>
                    <td>商品备注</td>
                    <td v-copy><span v-if="item.attribute">{{`[${item.attribute}]`}}</span>{{item.channel_sku_title}}</td>
...
<script>
    import {api_update,api_set_note,api_get_sku_siblings,url_update} from '../../../api/order-local';
    export default {
        permission: {
            url_update
        },
