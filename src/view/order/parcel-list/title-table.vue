<template>
    <div class="title-table-dialog">
        <page-dialog v-model="show" size="small" :title="title" class="table-container" @close="close" :close-on-click-modal="isClose">
            <el-table
                    ref="titleTable"
                    :data="title_data"
...
<script>
    export default {
        ...
        methods: {
            confirm() {
                this.$emit('export-title');
            },
            title_data_change(val) {
                this.$emit('check-title', val);
            },
            close() {
                this.$emit('clear-check-title');
            }
        },
        watch: {
            value(val) {
