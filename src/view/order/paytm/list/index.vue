<template>
    <page class="p-index">
        ...
        <verification-order v-model="verificationDialog" cur-platform="Paytm"></verification-order>
    </page>
</template>
<style lang="stylus">
...
<script>
    ...
    import {api_order_paytm_list,api_order_paytm_status} from'../../.././../api/order_paytm';
    export default{
        page:{
            devinfo:{
                frontEnd:'覃宏峰',
        ...
        methods:{
            ...
            get_status(){
                ...
                data.status = -1;
                this.$http(api_order_paytm_status,data).then(res=>{
                    res.forEach(btn=>{
            ...
            init(){
                this.loading=true;
                this.dataTable=[];
                this.$http(api_order_paytm_list,this.get_params()).then(res => {
