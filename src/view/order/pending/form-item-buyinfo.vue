<template>
    <el-row>
        ...
        <div class="absolute-f" v-if="edit&&form.channel_name==='ebay'" style="top:190px">
        ...
        <el-form label-width="120px" :model="form" ref="form" :rules="edit?rules:notRules">
            <el-form-item label="买家ID：">
                <el-input :disabled="true" v-if="edit" v-model="form.buyer" class="width-default"></el-input>
                <span v-else>{{form.buyer}}</span>
            </el-form-item>
            <el-form-item label="收件人：" prop="consignee">
                <el-input v-if="edit" v-model="form.consignee" class="width-default"></el-input>
                <span v-else>{{form.consignee}}</span>
            </el-form-item>
            <el-form-item label="地址：" prop="address">
                <el-input v-if="edit" v-model="form.address" class="width-default"></el-input>
                <span v-else>{{form.address}}</span>
            </el-form-item>
            <el-form-item label="地址2：" prop="address2">
                <el-input v-if="edit" v-model="form.address2" class="width-default"></el-input>
                <span v-else>{{form.address2}}</span>
            </el-form-item>
            <el-form-item  label="城市：" :prop="form.country_code!=='JP'?'city':''">
                <el-input v-if="edit" v-model="form.city" class="width-default"></el-input>
                <span v-else>{{form.city}}</span>
            </el-form-item>
            <el-form-item  label="省/州：">
                <el-input v-if="edit" v-model="form.province" class="width-default"></el-input>
                <span v-else>{{form.province}}</span>
            </el-form-item>
            <el-form-item label="国家：" prop="country_code">
                <el-select v-if="edit"  class="width-default"
                           v-model="form.country_code"
                           ref="country" filterable clearable
            ...
            <el-form-item label="邮编：" prop="zipcode">
                <el-input v-if="edit" v-model="form.zipcode" class="width-default"></el-input>
                <span v-else>{{form.zipcode}}</span>
            </el-form-item>
            <el-form-item label="联系电话：" prop="tel">
                <el-input v-if="edit" v-model="form.tel" class="width-default"></el-input>
                <span v-else>{{form.tel}}</span>
            </el-form-item>
            <el-form-item label="手机号：">
                <el-input v-if="edit" v-model="form.mobile" class="width-default"></el-input>
                <span v-else>{{form.mobile}}</span>
            </el-form-item>
            <el-form-item label="买家邮件：" prop="email">
                <el-input v-if="edit" v-model="form.email" class="width-default"></el-input>
    ...
    export default{
        ...
        data(){
            ...
            let checkChinese  = (rule,value,callback)=>{
                if(value){
                    return chinese.test(value)?callback(new Error("邮编格式有误！")):callback()
                }else{
                    return callback();
                }
            };
        ...
        methods:{
            ...
            use_paypal_source(){
                this.$http(api_use_paypal_source,this.id,{address:this.paypalSource}).then(res=>{
                    ...
                    this.form.address2 = this.paypalSource.address2;
            ...
            submit_params(isTrue){
              if(!this.form.mobile&&!this.form.tel)return this.$message({message:"请至少填写一种联系方式", type:'warning'});
              this.$refs.form.validate((bool)=>{
                if(bool){
                  ...
                  this.$http(api_update, this.id, params).then(res=>{
                    this.$message(res.message);
                    this.edit = false;
                    this.$emit('submit',updates,res.is_handle);
                  }).catch(code=>{
