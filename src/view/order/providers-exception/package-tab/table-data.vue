    ...
    <div class="c-table-data">
        ...
        <el-table
            ...
            <el-table-column label="平台订单号" inline-template width="210">
                <div v-for="(order,index) in row.order_list">
                    <ui-tip :key="index" :is-operate="true" :content="order.order_no"
                            @cur-click="get_order_detail(order)" :width="98"></ui-tip>
                </div>
            </el-table-column>
            <el-table-column label="包裹号" inline-template width="130">
                <div>
                    <ui-tip :is-operate="true" :content="row.package_number"
                            @cur-click="click_number(row.package_number)" :width="98"></ui-tip>
                </div>
            </el-table-column>
            <el-table-column label="面单号" inline-template width="140">
            ...
            <el-table-column label="预估重量(g)" inline-template width="100">
                <div>{{row.estimated_weight}}</div>
            </el-table-column>
            <el-table-column label="发货仓库" inline-template width="120">
                <div>{{row.warehouse_name}}</div>
            </el-table-column>
            <el-table-column label="异常原因" inline-template width="150">
                <span class="s-info-title" :title="row.exception_type_txt">{{row.exception_type_txt}}</span>
            </el-table-column>
            <el-table-column label="集包异常时间" inline-template width="150">
                <div>{{row.create_time | fdatetime}}</div>
            </el-table-column>
            <el-table-column label="剩余处理时间" inline-template width="150">
                <div :class="{red: row.remaining_time === '已超时'}">{{row.remaining_time}}</div>
            </el-table-column>
        </el-table>
        <el-pagination
                class="page-fixed"
        ...
        methods: {
            ...
            get_order_detail(order) {
                if (this.$hasPermission(this.apis.url_info)) {
                    ...
                    this.mdfTitle = `查看订单号: ${order.order_no} 信息`;
