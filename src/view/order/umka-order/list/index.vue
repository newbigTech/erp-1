...
<script>
    ...
    import {api_order_umka_list,api_order_umka_status} from'../../.././../api/order-umka';
    export default{
        page:{
            devinfo:{
                frontEnd:'熊辉',
        ...
        methods:{
            get_status(){
                ...
                data.status = -1;
                this.$http(api_order_umka_status,data).then(res=>{
                    res.forEach(btn=>{
            ...
            init(){
                this.loading=true;
                this.dataTable=[];
                this.$http(api_order_umka_list,this.get_params()).then(res => {
