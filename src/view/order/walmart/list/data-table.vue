        ...
        <ui-table v-loading="loading"
            ...
            <template v-for="(data, index) in dataTable">
                <tr class="line" :class="[data.show ? 'active' : '',data.clickData?'active-color':'']"
                    ...
                    <!--买家姓名-->
                    <td>
                        <ui-tip :content="data.customer_id" :width="72"></ui-tip>
                    </td>
                    <!--目的地-->
                    <td>
                    ...
                    <!--出售价-->
                    <td>
                        <ui-tip :content="data.sub_total" :width="88"></ui-tip>
                    </td>
                    <!--支付总额-->
                    <td>
                        <ui-tip :content="data.actual_total" :width="88"></ui-tip>
                    </td>
                    <!--付款时间-->
                    <td>
                        <ui-tip :content="data.order_created|dateFilter" :width="88"></ui-tip>
                    </td>
                    <!--平台发货状态-->
                    <td>
                        <ui-tip v-if="data.shipped_date>0" :content="data.shipped_date|dateFilter"
                                :width="88"></ui-tip>
                        <div v-else-if="data.shipped_date===0"><span>未发货</span></div>
                    </td>
                    <!--订单状态-->
                    <td>
                    ...
                    <!--最迟发货时间-->
                    <td>
                        <time-out :time="data.latest_shipped_time" :cur-status="data.shipped_date"></time-out>
                    </td>
                    <!--操作-->
                    <td>
                        <permission
                            tag="span"
                            :route="apis.url_api_order_walmart_check"
                            class="operate"
                ...
                <template v-if="data.show">
                    ...
                    <template v-for="(row,i) in data.sku_list">
                        <tr :key="`${i}${row.sku}${row.item_name}`">
                            <td colspan="2">
                                <ui-tip :content="row.sku" :width="88"></ui-tip>
                            </td>
                            <td colspan="8">
                                <ui-tip :content="row.item_name" :width="78"></ui-tip>
                            </td>
                            <td>{{row.qty}}</td>
...
<script>
    ...
    import {api_order_walmart_check, api_walmart_orders_export, url_walmart_orders_export} from '../../../../api/order_walmart';
    import {url_api_order_walmart_check} from '../../../../api/order_walmart';
    import uiTable from "../../../../components/ui-table.vue"

    export default {
        permission: {
            url_api_order_walmart_check,
            url_walmart_orders_export
        },
        data() {
            return {
                orders: false,
        ...
        methods: {
            ...
            lookOver(row) {//----------------------查看
                this.orders = true;
                this.checkList = {};
                this.$http(api_order_walmart_check, row.id).then(res => {
                    this.checkList = res;
