<template>
    <page class="p-index">
        ...
        <div class="block">
            ...
            <verification-order v-model="verificationDialog" cur-platform="Walmart"></verification-order>
        </div>
    </page>
</template>
<style lang="stylus">
...
<script>
    ...
    import {api_order_walmart_list,api_order_walmart_status} from'../../.././../api/order_walmart';
    export default{
        page:{
            devinfo:{
                frontEnd:'覃宏峰',
        ...
        methods:{
            ...
            get_status(){
                ...
                data.status = -1;
                this.$http(api_order_walmart_status,data).then(res=>{
                    res.forEach(btn=>{
            ...
            init(){
                this.loading=true;
                this.dataTable=[];
                this.$http(api_order_walmart_list,this.get_params()).then(res => {
            ...
            sort_click(val){
                switch (val.label){
                    case "支付总额":
                        this.searchData.sort_type="actual_total";
                        break;
                    case "付款时间":
                        this.searchData.sort_type="order_created";
                        break;
                    case "平台发货状态":
                        this.searchData.sort_type="shipped_date";
                        break;
                    case "下单时间":
                        this.searchData.sort_type="order_created";
                        break;
