        ...
        <ui-table v-loading="loading"
            ...
            <template v-for="(data, index) in dataTable">
                <tr class="line" :class="[data.show ? 'active' : '',data.clickData?'active-color':'']"
                    ...
                    <!--买家姓名-->
                    <td>
                        <ui-tip :content="data.buyer_last_name" :width="72"></ui-tip>
                    </td>
                    <!--目的地-->
                    <td>
                    ...
                    <!--运费-->
                    <td>
                        <ui-tip :content="data.delivery_total" :width="88"></ui-tip>
                    </td>
                    <!--出售价-->
                    <td>
                    ...
                    <!--付款日期-->
                    <td>
                        <ui-tip :content="data.time_created|timeFilter" :width="88"></ui-tip>
                    </td>
                    <!--平台发货状态-->
                    <td>
                        <ui-tip v-if="data.shipped_time>0" :content="data.shipped_time|dateFilter"
                                :width="88"></ui-tip>
                        <div v-else="data.shipped_time===0"><span>未发货</span></div>
                    </td>
                    <!--订单状态-->
                    <td>
                    ...
                    <!--最迟发货时间-->
                    <td>
                        <time-out :time="data.latest_shipped_time" :cur-status="data.shipped_time"></time-out>
                    </td>
                    <!--操作-->
                    <td>
                        <permission
                            tag="span"
                            :route="apis.url_api_order_yandex_check"
                            class="operate"
                ...
                <template v-if="data.show">
                    ...
                    <template v-for="(row,i) in data.sku_list">
                        <tr :key="`${i}${row.sku}${row.count}`">
                            <td colspan="2">
                                <ui-tip :content="row.sku" :width="88"></ui-tip>
                            </td>
                            <td colspan="8">
                                <ui-tip :content="row.offer_name" :width="78"></ui-tip>
                            </td>
                            <td>{{row.count}}</td>
                            <td>{{row.price}}</td>
                        </tr>
...
<script>
    ...
    import {api_order_yandex_check} from '../../../../api/order-yandex';
    import {url_api_order_yandex_check} from '../../../../api/order-yandex';
    import uiTable from "../../../../components/ui-table.vue"

    export default {
        permission: {
            url_api_order_yandex_check,
        },
        data() {
            return {
                orders: false,
        ...
        methods: {
            ...
            lookOver(row) {//----------------------查看
                this.orders = true;
                this.checkList = {};
                this.$http(api_order_yandex_check, row.id).then(res => {
                    this.checkList = res;
