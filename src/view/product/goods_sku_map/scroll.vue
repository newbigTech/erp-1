    ...
    <div class="scroll-select">
        <div class="input">
            <input ref="input"
                   ...
                   @input="input"
                   :disabled="disable">
            <i v-show="!show" class="active el-icon-caret-bottom" style="right:0;" @click="()=>{$refs.input.focus()}"></i>
            <i v-show="show" class="active el-icon-caret-left" style="right:0;" @click="show=false;"></i>
        </div>
        <transition name="md-fade-bottom" tag="li">
            <div class="body" v-show="show" :style="styleUlOn" @scroll="scroll" @mousewheel="mousewheel" ref="body">
                ...
                <div key="count  > (maxPage * size)" v-if="count  > (maxPage * size)" style="color:#998220;" :style="{textAlign}"  class="other-class">数据太多请</div>
                <div style="font-weight:bold;text-align:center" class="other-class" key="count > size" v-else-if="count > size">
                    <span>总数量{{count}};当前{{page}}</span>
                </div>
    ...
    export default{
        ...
        methods:{
            refresh_remote(){
                ...
                return this.$http(http('GET', this.remote, this.fixParams({page:this.page,pageSize:this.pageSize,keyword:this.inputValue}))).then(res=>{
            ...
            input(event){
                ...
                this.timer = setTimeout(()=>{
                    ...
                    this.refresh_remote().then(()=>{
                        this.$nextTick(()=>{
                            let find = this.options.find(row=>row.label === event.target.value);
                            if(find){
                                this.val = find.label;
