    ...
    <page class="p-index">
        ...
        <div class="mt-sm mb-sm ml-sm">
            <permission tag="ElButton"
                        ...
                        :route="apis.url_add_develop_department">添加部门</permission>
            <permission tag="ElButton"
                        type="primary" size="mini" class="inline"
                        @click.native="batch_set_target"
                        :route="apis.url_develop_save_import">批量设置成员考核目标</permission>
        </div>
        ...
        <add-member v-model="memberDialog"
                    ...
                    :cur-page="curPage"
                    :id="curId"></add-member>
        <batch-set-target v-model="setTargetDialog"
                          :cur-page="curPage"></batch-set-target>
...
<script>
    import {api_develop_target_department,api_get_get_gepartment,api_develop_target_user,
        url_add_develop_department,url_develop_save_import} from '@/api/target-configure';
    export default {
        permission:{
            url_add_develop_department,
            url_develop_save_import
        },
        data() {
            return {
                curPage:'develop',
        ...
        methods:{
            ...
            init(){
                if(this.searchData.snType===1){
                    ...
                    this.search_department_group(params);
                }else{
                    let params = {
                        department_id:'',
            ...
            //部门||小组的搜索
            search_department_group(params){
                this.$http(api_develop_target_department,params).then(res => {
                    console.log(res,'res');
        ...
        components: {
            ...
            addDepartment:require('../target-configure/add-department.vue').default,
            addGroup:require('../target-configure/add-group.vue').default,
            addMember:require('../target-configure/add-member.vue').default,
            tableData:require('./table-data').default,
            batchSetTarget:require('../target-configure/batch-set-target.vue').default,
            editMember:require('../target-configure/edit-member.vue').default,
