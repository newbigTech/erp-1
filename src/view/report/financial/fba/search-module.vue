    ...
    <div>
        <el-card>
            ...
            <label-item label="发货仓库：" class="ml-sm mr-sm">
                <el-select class="s-width-default" v-model="searchData.warehouse_id" filterable clearable>
                    <el-option v-for="item in warehouseList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
            </label-item>
            <label-item class="mr-sm">
                <el-select v-model="searchData.time_field" class="s-width-small">
            ...
            <el-button @click.native="clearSearch(searchData)" class="inline" size="mini">清空搜索</el-button>
        </el-card>
    </div>
</template>
<style lang="stylus">
