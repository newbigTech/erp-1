<template>
    <div>
        <indent-table v-loading="loading"
                      ...
                      :heads="[
                    //注意：更改此处size的时候需要更改下面对应的size并且还要去ui-table的style里改总宽度
                    {label:'订单号',size:301},
...
<script>
    import {api_report_walmart,api_get_orders_skus} from  '../../../../api/report-channel'
    export default {
        data(){
            return {
                tableData:[],
        ...
        methods: {
            init(){
                ...
                this.$http(api_report_walmart, data).then(res=>{
                    this.loading = false;
