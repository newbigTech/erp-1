import {api_get_target_user} from '@/api/target-configure';
import {edit_target_department,add_target_department,url_add_user,url_update_target_user} from '@/api/target-configure';
//部门下的操作
const operate = [
    {label:'编辑',api:edit_target_department,action(row){
            this.edit(row);
    }},
    {label:'添加分组',api:add_target_department,action(row){
        this.add_group(row)
    }},
];
//组下的操作
...
const groupOperate2 = [
    ...
    {label:'添加成员',api:url_add_user,action(row){
        this.add_member(row)
    }},
];
//成员下的操作
const memberOperate = [
    {label:'编辑',api:url_update_target_user,action(row){
...
export default{
    ...
    methods:{
        ...
        test(rowData){
            this.$http(api_get_target_user,{department_id:rowData.id}).then(res => {
        ...
        return (
                <ui-table body-height={10}
                          heads={[{label:'ID',width:5},
                                {label:'部门名称',width:26},
                                {label:'部门类型',width:6},
                                {label:'部门负责人',width:13},
                                {label:'销售额目标（$）',width:15},
                                {label:'状态',width:10},
                                {label:'菜单操作',width:15}]}>
                    <tbody>
                        {childs}
