<template>
    <div class="c-card-search p-package-tab-search">
        <search-card :params="form" :clears="clears" @search="search">
            ...
            <label-item label="发送者：" class="ml-sm">
                <scroll-select v-model="sendUser"
                               style="width:120px"
                               class="inline"
                               textAlign="left"
                               v-sf.send_id
    ...
    export default {
        ...
        methods: {
            ...
            change_channel(index, item) {
                this.buttonsChangeCount++;
                this.btnVal.read = item.value;
                if(this.buttonsChangeCount>1){
                    this.$emit('search');
                }
            },
            fix_params_account({page,pageSize,keyword}){
                if(!keyword){
                    this.sendUser = {label:'',value:''};
                }
                return {
                    page:page,
                    pageSize:pageSize,
        ...
        computed:{
            sendUser:{
                get(){
                    return {label:this.sender,
                        value:this.form.send_id}
                },
                set(val){
                    this.sender = val.label;
                    this.form.send_id = val.value;
                }
            }
        },
        watch:{
