<template>
    <page-dialog :title="dummyTittle" v-model="dialog"
        ...
        <div>
            <dummy-header :form="form" :current-handle="currentHandle" :channel-list="channelList"></dummy-header>
        </div>
        <!--添加申请时显示的按钮-->
        ...
        <dummy-table :form="form"
                     ...
                     :type-options="typeOptions"></dummy-table>
...
import {api_virtual_order,api_post_virtual_order,api_look_over,api_audit_minister,api_audit_headman,api_batch_cancel,api_virtual_order_import} from '../../../api/request-note';
import {url_audit_minister,url_audit_headman,url_batch_cancel,url_virtual_order_import} from '../../../api/request-note';
    export default {
        permission:{
            url_audit_minister,
            url_audit_headman,
            ...
            comTotalCost(){
                ...
                if(this.form.detail&&this.form.detail.length>0){
                    this.form.detail.forEach(row=>{
                        cur += Number(row.time_quantity.estimate_cost);
                    })
                }
                return cur;
            },
        ...
        methods:{
            ...
            export_excel(val){
                val.forEach(row=>{
                    ...
                    cur.is_add_shopping_cart = !!cur.is_add_shopping_cart?true:false;
                    cur.is_add_wishlist = !!cur.is_add_wishlist?true:false;
            ...
            add_goods(val){
                val.forEach(row=>{
                    ...
                    let path = [
                        {path:row.path,base_url:this.baseUrl}
                    ];
                    let obj = {
                        sku_id:row.sku_id,
                        sku:row.sku,
                        ...
                        product_location:"",
                        product_link:"",
                        time_quantity:[{task_time:date,quantity:'',seller_cost: '',estimate_cost: '',order_currency:'',msg_time:''}],
                        is_collection_product:false,
                        is_stars:false,
                        is_collection_shop:false,
                        is_add_shopping_cart:false,
            ...
            submit_approve(){
                ...
                cloneData.detail.forEach((row,index)=>{
                    ...
                    row.is_add_shopping_cart = row.is_add_shopping_cart?1:0;
                    row.is_add_wishlist = row.is_add_wishlist?1:0;
                    if(row.isSaveBtn){
                        canSubmit = false;
                    ...
                    row.time_quantity.forEach((res,i)=>{
                        let date1 = Math.floor(new Date(cur.time_quantity[i].task_time).getTime()/1000);
                        let date2 = Math.floor(new Date(cur.time_quantity[i].msg_time).getTime()/1000);
                        res.task_time = date1;
                        res.msg_time = date2
                    });
                ...
                this.$http(api_look_over,this.id).then(res=>{
                    ...
                    this.$nextTick(()=>{
                        res.detail.forEach(row=>{
                            ...
                            row.is_add_shopping_cart = row.is_add_shopping_cart?true:false;
                            row.is_add_wishlist = row.is_add_wishlist?true:false;
                        });
